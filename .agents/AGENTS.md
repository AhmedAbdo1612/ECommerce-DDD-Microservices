...
8. **React Frontend Styling:** Use native inline Vanilla CSS mapped to modern design aesthetics. **Do not install Tailwind CSS globally**, as its preflight CSS overrides and breaks the existing layout and theme engine. Implement Tailwind-like designs (gradients, rounded pill badges, ghost buttons) using pure inline CSS or the existing `theme` object.
9. **Role-Based Access Control (RBAC):** Users can hold multiple roles (e.g., `["Admin", "Customer"]`). Frontend route protection and context flags (`isAdmin`, `isManager`) must always evaluate role intersections using array methods rather than strict string equality.
